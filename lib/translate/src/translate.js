...
if (!process.argv[2])
  throw new Error(`[ERROR] Missing argument: source directory`);
...
async function* generateTranslationTargets() {
  for (const filename of readDir(SRC_DIR)) {
    if (path.extname(filename) === ".html") {
      const extractionStream = extractHTMLTranslationTargets(filename);
      for await (const target of extractionStream) {
        yield target;
      }
    }
  }
...
async function translate() {
  const combinedStream = Readable.from(generateTranslationTargets());
  await pipeline(combinedStream, processorStream, verboseStream);
}
...
async function processTranslationTarget(translationTarget) {
  ...
  if (!translation) {
    if (
      !TRY_TRANSLATE ||
      pendingTranslations.has(translationTarget.key) ||
...
const processorStream = new Transform({
  ...
  flush(callback) {
    // In a Transform stream, 'final' is called 'flush'
    try {
